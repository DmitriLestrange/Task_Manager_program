...
def add_no_store_headers(response):
    if request.path.startswith(SESSION_SENSITIVE_PATH_PREFIXES):
...
def clear_session_response(payload: dict[str, Any], *, status_code: int = 200):
    session.clear()
...
def delete_profile():
    ...
    with get_db() as conn:
        ...
        if not user:
            return clear_session_response({"error": "User not found."}, status_code=404)
    ...
    return clear_session_response({"message": "Account deleted successfully."})


@app.route("/api/auth/me", methods=["GET"])
def me():
    if "user_id" not in session:
        return clear_session_response({"user": None})
    ensure_db()
    with get_db() as conn:
        user = conn.execute(
            "SELECT id, username, contact, profile_image_path, is_admin FROM users WHERE id = ?",
