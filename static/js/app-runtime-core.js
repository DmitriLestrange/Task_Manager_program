...
async function fetchJSON(url, options = {}) {
    const sessionSensitive = typeof url === "string" && (
    ...
    const response = await fetch(url, {
        headers: {
            ...
            ...(sessionSensitive ? { "Cache-Control": "no-cache", Pragma: "no-cache" } : {}),
            ...(options.headers || {}),
        },
        credentials: "same-origin",
        cache: sessionSensitive ? "no-store" : (options.cache || "default"),
        ...options,
    });
...
function clearUserScopedStorage(userId) {
    ...
    if (userId !== null && userId !== undefined && Number.isFinite(Number(userId))) {
        keys.add(`tm_seen_mentions_${Number(userId)}`);
        keys.add(`tm_collapsed_tasks_${Number(userId)}`);
        keys.add(`tm_pinned_task_${Number(userId)}`);
        keys.add(`tm_pinned_chat_${Number(userId)}`);
    }
    keys.forEach((key) => localStorage.removeItem(key));
}

function resetClientAuthState(options = {}) {
    const {
    ...
    editingTaskId = null;
    userProfileReturnPage = "dashboardPage";
    ...
    if (privateMessageBadge) {
        privateMessageBadge.textContent = "0";
